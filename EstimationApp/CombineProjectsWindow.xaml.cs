...
{
    public class CombineBuildingItem
    {
    ...
    public partial class CombineProjectsWindow : Window
    {
        public ObservableCollection<CombineBuildingItem> SelectedBuildings { get; set; } = new ObservableCollection<CombineBuildingItem>();

        public CombineProjectsWindow()
        {
            InitializeComponent();
        ...
        private void BtnAddFiles_Click(object sender, RoutedEventArgs e)
        {
            OpenFileDialog openFileDialog = new OpenFileDialog
            {
                ...
                Multiselect = true,
                Title = "Select Projects/Databases to Combine"
            };
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                foreach (string file in openFileDialog.FileNames)
                {
                    try
                    {
                        ...
                        if (proj != null)
                        {
                            // Extract buildings with their quantities from the source project
                            var sourceBuildings = proj.Buildings?.Where(b => !string.IsNullOrWhiteSpace(b.BuildingName)).ToList() ?? new List<BuildingGroup>();

                            if (sourceBuildings.Count == 0 && proj.WorkItems != null)
                            {
                                var fallbackNames = proj.WorkItems.SelectMany(w => w.Sheets).Select(s => s.BuildingName).Where(b => !string.IsNullOrWhiteSpace(b)).Distinct();
                                sourceBuildings = fallbackNames.Select(name => new BuildingGroup { BuildingName = name, NumberOfBuildings = 1 }).ToList();
                            }

                            foreach (var bGrp in sourceBuildings)
                            {
                                if (!SelectedBuildings.Any(x => x.BuildingName == bGrp.BuildingName && x.FilePath == file))
                                {
                                    SelectedBuildings.Add(new CombineBuildingItem
                                    {
                                        BuildingName = bGrp.BuildingName,
                                        NumberOfBuildings = bGrp.NumberOfBuildings, // Pulls the exact quantity
                                        FilePath = file,
                                        ParsedProject = proj
                                    });
                                }
        ...
        {
            if (GridBuildings.SelectedItem is CombineBuildingItem selectedItem)
            {
        ...
        private void BtnCombine_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("Please add at least one building to combine.", "No Data", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            try
            {
                ProjectDetails masterProject = new ProjectDetails
                {
                    ProjectName = "Combined Workspace - " + DateTime.Now.ToString("MMM dd"),
                    OwnerName = "Combined Projects",
                    WorkItems = new ObservableCollection<WorkItemEntry>(),
                    Buildings = new ObservableCollection<BuildingGroup>()
                };
                ...
                foreach (var bldgItem in SelectedBuildings)
                {
                    ...
                    if (isFirstFile)
                    {
                        masterProject.MasterWork = proj.MasterWork;
                        masterProject.MasterItems = proj.MasterItems;
                        masterProject.MasterSubItems = proj.MasterSubItems;
                        masterProject.MasterUnits = proj.MasterUnits;
                        masterProject.MaterialTests = proj.MaterialTests;
                        masterProject.Budget = proj.Budget;
                        isFirstFile = false;
                    }
                    else
                    {
                    ...
                    // CRITICAL FIX: Retain the imported building quantity instead of resetting it to 1
                    masterProject.Buildings.Add(new BuildingGroup
                    {
                        BuildingName = finalName,
                        NumberOfBuildings = bldgItem.NumberOfBuildings
                    });
                    ...
                    if (proj.WorkItems != null)
                    {
                        foreach (var wi in proj.WorkItems)
                        {
                            ...
                            if (targetSheets.Count > 0)
                            {
                                ...
                                if (masterWi == null)
                                {
                                    masterWi = new WorkItemEntry
                                    {
                                        SubItemID = wi.SubItemID,
                                        DisplayWorkName = wi.DisplayWorkName,
                                        DisplayItemName = wi.DisplayItemName,
                ...
                masterProject.PromptFloorMappingIfNeeded();

                AppState.CurrentProject = masterProject;
                WorkEnvironmentWindow workspace = new WorkEnvironmentWindow();
                workspace.Show();
            ...
            {
                MessageBox.Show($"Critical error combining data: {ex.Message}", "Merge Failed", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
